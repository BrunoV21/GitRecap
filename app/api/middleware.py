    ...
    if authorization_header:
        ...
        if token_prefix in authorization_header:
            ...
            with db.get_session() as session:
                oauth_token_db_record = session.query(models.OAuthToken).filter_by(access_token=access_token).first()
                if oauth_token_db_record:
                    ...
                    if oauth_token_db_record.expires_at <= datetime.utcnow():
                        ...
                        print("The access token seems to have expired, based on the saved data in the db record.")
                    ...
                    if user_should_still_be_signed_in:
                        ...
                        if user:
                            ...
                            return web.Response(status=500, reason='We have a record of the supplied access token and it '
                                                                   'has an associated user ID, but we couldn\'t find the '
                                                                   'associated user record with that ID.')
                else:
                    print("Unable to find a db record for the supplied access token.")
        else:
            print("The request has an Authorization header, but it doesn't seem to have a value containing the "
                  "necessary 'token ' prefix.")
...
def get_new_access_token(oauth_token_db_record, session):
    ...
    we have is always up-to-date (not expired).  Access tokens seem to last eight hours, at which point, if we try to
    get a new access token and the refresh token has been revoked, we'll know the user has signed out remotely
    ...
    if the_refresh_token_is_invalid:
        ...
        oauth_token_db_record.refresh_token_expires_at = datetime.utcnow() + timedelta(seconds=response_data['refresh_token_expires_in'])
